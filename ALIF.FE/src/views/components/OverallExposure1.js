...
import { getSupplierDetailsAsync } from '../../redux/async/vcn/vcn.async'
...
var pointSeries
const OverallExposure1 = (filters) => {
  const { supplierConcentrationData, supplierConcentrationLoader } = useSelector(
    (state) => state.supplierconcentration,
  )
  const { supplierDetailsData } = useSelector((state) => state.supplierdetails)
  const dispatch = useDispatch()
  const navigate = useNavigate()

  useEffect(() => {
    let data1 = []
    for (let i = 0; i < supplierConcentrationData.length; i++) {
      data1.push({
        id: supplierConcentrationData[i].country_code,
        name: supplierConcentrationData[i].country,
        value: supplierConcentrationData[i].supplier_count,
        score: supplierConcentrationData[i].avg_score,
      })
    }
    let root = am5.Root.new('overallExposure1')
    root._logo.dispose()
    ...
    worldSeries.mapPolygons.template.events.on('click', (ev) => {
      ...
      Promise.all([
        ...
        let riskLevelData = []
        for (let i = 0; i < filters.data.risk_level.length; i++) {
          riskLevelData.push(filters.data.risk_level[i].value)
        }
        let riskTypeData = []
        for (let i = 0; i < filters.data.risk_type.length; i++) {
          riskTypeData.push(filters.data.risk_type[i].value)
        }
        let risklevels = ''
        ...
        let data1 = {
          product_id: filters.data.product_id,
          risk_level: risklevels,
          risk_type: risktypes,
          country_code: data.id,
        }
        dispatch(getSupplierDetailsAsync(data1))
    ...
    const getColor = (code) => {
      for (let i = 0; i < data1.length; i++) {
        if (data1[i].name == code) {
          if (data1[i].score <= 30) {
            return colors[0]
          } else if (data1[i].score <= 70) {
            return colors[1]
          } else if (data1[i].score <= 100) {
            return colors[1]
          } else {
            return colors[0]
          }
    ...
    pointSeries.bullets.push(function () {
      ...
      circle.events.on('dblclick', function (ev) {
        dispatch(setShipperID(ev.target.dataItem._settings.shipper_id))
        navigate('/scorecard')
      })
    ...
    if (supplierConcentrationData.length > 0) {
      return () => {
        root.dispose()
      }
    }
  ...
  useEffect(() => {
    for (let i in supplierDetailsData) {
      let s = supplierDetailsData[i]
      const singleRowData = [
        {
          latitude: s.lat,
          longitude: s.lng,
          country: s.country_code,
          company_name: s.shipper_company_name,
          value: 50,
          risk_level: s.avg_risk_level,
          score: s.avg_score,
  ...
  return (
    <>
      {supplierConcentrationLoader ? (
        <SkeletonLoading width="100%" height="100%" radius="0px" />
      ) : (
        <CRow className="m-0 p-3 h-100">
          <div id="overallExposure1" style={{ width: '98%', height: '100%' }}>
            {supplierConcentrationData.length > 0 ? <></> :
              <Empty />}
          </div>
        </CRow>
      )}
...
export default OverallExposure1
