...
Param(
    ...
    [string] $TemplateFile = '.\Artifacts\azuredeploy.json',
    [string] $TemplateParametersFile = '.\Artifacts\azuredeploy.parameters.json',
...
if ($UploadArtifacts) {
    ...
    # If using parameter file, parse the parameter file and update the values of artifacts location and artifacts location SAS token if they are present
    if ($TemplateParametersFile) {
    ...
    if ($StorageAccount -eq $null) {
        ...
        New-AzResourceGroup -Location "$ResourceGroupLocation" -Name $StorageResourceGroupName -Tag $Tags -Force
...
Write-Output "optional param" $OptionalParameters
# Create or update the resource group using the specified template file and template parameters file
New-AzResourceGroup -Name $ResourceGroupName -Location $ResourceGroupLocation -Tag $Tags -Verbose -Force
