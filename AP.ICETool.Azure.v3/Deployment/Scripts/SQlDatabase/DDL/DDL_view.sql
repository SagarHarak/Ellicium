...
(
	...
	AND md.date_diff=md.min_date
	 LEFT JOIN viz.tbl_all_commodities_data bcd
	 ON bcd.Index_Ticker = ces.index_ticker
	  AND bcd.Date = md.date) SELECT
		map.[fiscal_year]
		,map.[fiscal_month]
