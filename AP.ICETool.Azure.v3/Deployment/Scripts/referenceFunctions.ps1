...
function Build-PowerBIWorkspaceName ([string]$ProjectCodeName, [string]$Environment) {

    $workspaceName = "ENG-" +$ProjectCodeName + "-" + $environmentAbbreviations[$Environment]
    return $workspaceName
}

#Function to build Resource Group names per the AP standard
function Format-ResourceGroupName() {
...
# Function to Assign Group to Enterprise Application
function Add-GroupToEnterpriseApplication ([string]$GroupId, [string]$EnterpriseApplicationName) {
    ...
    $EntAppGroupAssigncheck = (Get-MgServicePrincipalAppRoleAssignedTo `
                                -ServicePrincipalId $EntApp_Id).PrincipalId
